...
/**
 * @Description: 不可达原因-adapter
 * @Author: zzh
 * @CreateDate: 2021/12/15
 */
public class SelectUnreachReasonAdapter extends BaseEditSpinnerAdapter implements EditSpinnerFilter {

    private Context mContext;
    private List<String> mList;

    public SelectUnreachReasonAdapter(Context context, List<String> list) {
        super(list);
    ...
    public String getItem(int position) {
        return mList == null ? "" : mList.get(position) == null ? "" : mList.get(position);
    }
...
//        textView.setText(Html.fromHtml(getItem(position)));
        viewHolder.tv_name.setText(StringUtils.nullStrToEmpty(mList.get(position)));
        return convertView;
    }
